...
async def chat(
    ...
    results = retrieval.search(db, embedder, request.message, request.top_k)
    ...
    return ChatResponse(reply=reply, model=request.model or settings.llm_model, sources=results)

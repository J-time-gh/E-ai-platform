...
同一语料（3 篇 / 53 chunk）、同一题库（20 条：15 可答 + 5 库外）、同一生成模型：

| 检索策略 | hit@5 | hit@1 | MRR | 拒答正确率 |
|---|---|---|---|---|
| 纯向量（基线） | 86.7% | 60.0% | 0.722 | 100% |
| BM25 | **93.3%** | **86.7%** | **0.889** | 待修 |
| **两路并集（RRF 理论上限）** | **100%** | — | — | — |

关键发现：BM25 精确救回向量漏掉的「科学实验」「拜物教」（字面匹配），
向量独有命中「《实践论》批判了哪两种错误倾向」（语义匹配），两者互补。
引入检索层门控后，库外请求延迟从 3~6s 降到 0.04s（不调用大模型）。
